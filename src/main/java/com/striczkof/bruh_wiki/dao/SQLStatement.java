...
enum SQLStatement {
    ...
    // Constant names: table name_(admin if admin only)_(make/get/set/del)_(specific columns)_number_modifier_(by_value for where except none or only id)
    // User stuff
    USERS_GET_COUNT("SELECT COUNT(id) FROM users"),
    ...
    // Article stuff
    ART_GET_COUNT("SELECT COUNT(id) FROM articles"),
    ART_GET_ONE("SELECT id, category_id, UNIX_TIMESTAMP(made) AS 'made', UNIX_TIMESTAMP(lastEdited) AS 'lastEdited', title, content, hidden  FROM articles WHERE id=?"),
    ART_GET_ONE_TRUNC("SELECT id, category_id, UNIX_TIMESTAMP(made) AS 'made', UNIX_TIMESTAMP(lastEdited) AS 'lastEdited', title, CASE WHEN CHAR_LENGTH(content) > ? THEN CONCAT(SUBSTRING(content, 1, ?), '...') ELSE content END AS 'content', hidden FROM articles ORDER BY lastEdited DESC"),
    ART_GET_ALL("SELECT id, category_id, UNIX_TIMESTAMP(made) AS 'made', UNIX_TIMESTAMP(lastEdited) AS 'lastEdited', title, content, hidden FROM articles ORDER BY lastEdited DESC"),
    ART_GET_ALL_TRUNC("SELECT id, category_id, UNIX_TIMESTAMP(made) AS 'made', UNIX_TIMESTAMP(lastEdited) AS 'lastEdited', title, CASE WHEN CHAR_LENGTH(content) > ? THEN CONCAT(SUBSTRING(content, 1, ?), '...') ELSE content END AS 'content', hidden FROM articles ORDER BY lastEdited DESC"),
    ART_GET_SOME_BY_EDITED_RANGE("SELECT id, category_id, UNIX_TIMESTAMP(made) AS 'made', UNIX_TIMESTAMP(lastEdited) AS 'lastEdited', title, content, hidden FROM articles ORDER BY lastEdited DESC LIMIT ? OFFSET ?"),
    ART_GET_SOME_TRUNC_BY_EDITED_RANGE("SELECT id, category_id, UNIX_TIMESTAMP(made) AS 'made', UNIX_TIMESTAMP(lastEdited) AS 'lastEdited', title, CASE WHEN CHAR_LENGTH(content) > ? THEN CONCAT(SUBSTRING(content, 1, ?), '...') ELSE content END AS 'content', hidden FROM articles ORDER BY lastEdited DESC LIMIT ? OFFSET ?"),
    ART_GET_SOME_BY_MATCHING("SELECT articles.id, title, categories.name, CASE WHEN CHAR_LENGTH(content) > ? THEN CONCAT(SUBSTRING(content, 1, ?), '...') ELSE content END AS 'content', MATCH (title, content) AGAINST (? WITH QUERY EXPANSION) AS 'relevance' FROM articles LEFT JOIN categories ON articles.category_id = categories.id WHERE hidden IS FALSE AND MATCH (title, content) AGAINST (? WITH QUERY EXPANSION) ORDER BY 'relevance' DESC LIMIT ?"),
    ...
    // Category stuff
    CAT_GET_COUNT("SELECT COUNT(id) FROM categories"),
